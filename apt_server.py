...
class SimpleHTTPRequestHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        f.write(b"<html>\n<title>Turbo APT</title>\n")
        f.write(b"<body>\n<h2>Turbo APT Server</h2>\n")
        f.write(b"<hr>\n")
        if r:
    ...
    def send_head(self):
        ...
        if self.path.startswith("/get_data"):
            print("Get data! Serving JSON data.")
            params = dict(urllib.parse.parse_qsl(urllib.parse.urlsplit(self.path).query))
            if params:
                print("Got some additional params to filter by.")
                if "apt_id" in params:
                    with open(_g_apt_file, "r") as json_file:
                        apts_dict = json.load(json_file)
                        output_dict = [x for x in apts_dict if str(x['apt_id']) == str(params["apt_id"])]
                        f = BytesIO()
                        f.write(json.dumps(output_dict, indent=4).encode('utf-8'))
                        self.send_headers_for_file(f, "application/json", physical_file=False)
                        return f
            else:
                print("Returning all data.")
                local_path = os.path.abspath(_g_apt_file)
        elif self.path.startswith("/add_data"):
            print("Adding data!")
            params = dict(urllib.parse.parse_qsl(urllib.parse.urlsplit(self.path).query))
        ...
        #In this case path will be a local filename
        ...
        try:
            ...
            return None
        self.send_headers_for_file(f, ctype)
        return f
 
    def send_headers_for_file(self, f, ctype, physical_file=True):
        self.send_response(200)
